...
import { createCourierForCompany, listCouriersForCompany } from "@/src/server/domain/couriers/service";
...
export async function POST(
  ...
  try {
    const courier = await createCourierForCompany(dealer.id, parseCourierInput(body));
    return NextResponse.json({ ok: true, courier }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Kurye oluşturulamadı";
